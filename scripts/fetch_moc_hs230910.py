...
def build_country_metadata(country_rows: list[dict[str, Any]], groups: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]], dict[str, dict[str, str]]]:
    country_by_id = {
        str(row["id"]): {
...
def make_payload(year: int, month: int, year_name: Any, hs_name: str) -> dict[str, Any]:
    return {
...
def main() -> None:
    ...
    hs_lookup = request_json(session, "POST", f"{BASE_URL}/lookup/hscodelist{HS_CODE}/{HS_VERSION}", json_payload={"lang": "th"})
    hs_name = hs_lookup.get("Name", HS_CODE)
    ...
    for index, (year, month) in enumerate(periods, start=1):
        payload = make_payload(year, month, year_name_by_id.get(str(year), year + 543), hs_name)
